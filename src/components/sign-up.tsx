...
const SignUp: React.FC<Props> = ({ className, modeToggle, login }) => {
  ...
  const onInvalid = () => {
    ...
    else if (errors.password && errors.password.type === 'minLength') toast.error('Password должен быть больше 6 символов')
        ...
        <form onSubmit={handleSubmit(onSubmit, onInvalid)}>
          <div className="flex flex-col gap-6">
            ...
            <div className="grid gap-2">
              ...
              <Input id="password" type="password" placeholder='Придумайте пароль' {...register('password', { required: true, minLength: 6 })} />
            </div>
            <div className="grid gap-2">
              <span className=''>Repeat password</span>
              <Input id="repeatPassword" type="password" placeholder='Повторите пароль' {...register('repeatPassword', { required: true, minLength: 6, validate: (value) => value === passwordValue })} />
            </div>
          ...
          <div className="mt-4 text-center text-sm">
            ...
            <span className="underline underline-offset-4 cursor-pointer" onClick={modeToggle}>
              Войти
            </span>
          </div>
        </form>
